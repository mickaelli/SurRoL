...
class SurRoLEnv(gym.Env):
    ...
    def __init__(self, render_mode: str = None):
        ...
        if render_mode == 'human':
            ...
            # EGL renderer is optional; try to load it if available, but don't
            # hard-fail on machines without the plugin (e.g. remote servers).
            global _WARNED_EGL
            try:
                ...
                if not _WARNED_EGL:
                    _WARNED_EGL = True
                    print(f"[SurRoL] EGL renderer plugin not available: {exc}")
